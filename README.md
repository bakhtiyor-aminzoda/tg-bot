...
1. В `.env` задайте переменные:
  ...
  ```
2. Запустите бота как обычно. При поднятии появится новый сервер на `http://127.0.0.1:8090/admin`.
3. Добавьте `?token=supersecret` к URL или передавайте заголовок `X-Admin-Token: supersecret`, чтобы пройти авторизацию.
...
Форма в шапке позволяет фильтровать статистику по `chat_id`, искать чат по названию и переключать сортировку всех разделов. Панель работает только при включённой истории (`ENABLE_HISTORY=true`). Health-блок отображается, если `HEALTHCHECK_ENABLED=true`.

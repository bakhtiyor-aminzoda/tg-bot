...
- `/profile` показывает текущий тариф, использование дневных/месячных лимитов, активные бонусы и время следующего сброса.
- `/referral` открывает вкладку реферальной программы с кнопками навигации. Пользователи получают личную ссылку вида `https://t.me/<BOT_USERNAME>?start=ref_<код>` и могут делиться ей одной кнопкой.
- При переходе по такой ссылке код применяется автоматически во время `/start` (без ручного ввода).
- Чтобы ссылка собиралась корректно, укажите `BOT_USERNAME` в `.env` (без `@`).
...
- `BOT_USERNAME` — Telegram-username бота без `@`. Используется для генерации реферальных ссылок и кнопки “Поделиться ссылкой” в `/profile`.
